...
/// A ringbuffer for perf events generated by the kernel.
...
impl<'a> Builder<'a> {
    ...
    /// Construct a [`Counter`] and a [`SampleBuffer`] according to the
    /// specifications made on this builder.
    /// 
    /// A freshly built [`Counter`] is disabled. To begin counting events, you
    /// must call [`enable`] on the [`Counter`] or the [`Group`] to which it
    /// belongs.
    /// 
    /// If the `Builder` requests features that the running kernel does not
    /// support, it returns `Err(e)` where `e.kind() == ErrorKind::Other` and
    /// `e.raw_os_error() == Some(libc::E2BIG)`.
    /// 
    /// Unfortunately, the problems in counter configuration are detected at this
    /// point, by the kernel, not earlier when the offending request is made on
    /// the `Builder`. The kernel's returned errors are not always helpful.
    /// 
    /// [`enable`]: crate::Counter::enable
    pub fn sample(mut self, map_size: usize) -> std::io::Result<(Counter, SampleBuffer)> {
        let page_size = match unsafe { libc::sysconf(libc::_SC_PAGE_SIZE) } {
            page_size if page_size < 0 => return Err(std::io::Error::last_os_error()),
            page_size => page_size as usize,
        };

        let counter = self.build_raw()?;

        let map_size = map_size.next_power_of_two() + page_size;
        let map_size = (map_size - 1) / page_size * page_size;
        let map = unsafe { MmapOptions::new().len(map_size).map_mut(&counter.file)? };

        let header = map.as_ptr() as *const perf_event_mmap_page;
        let buffer = unsafe { map.as_ptr().add((*header).data_offset as _) };
    ...
    fn build_raw(&mut self) -> std::io::Result<Counter> {
